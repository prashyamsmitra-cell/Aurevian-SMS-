...
export default function TeacherNotes() {
  ...
  function submit(e: React.FormEvent) {
    ...
    addNote({ classId, className: cls.name, title, fileName: fileName || `${title.toLowerCase().replace(/\s+/g, '-')}.pdf`, uploadedByName: session.name, uploadedByRole: 'teacher' })
    setModalOpen(false)
  ...
  return (
    <div>
      <SectionHeading
        eyebrow={`${myNotes.length} notes shared`}
        title="Notes"
        subtitle="Share lecture material with students enrolled in your classes."
        action={<Button onClick={() => setModalOpen(true)} disabled={myClasses.length === 0}><Upload size={15} /> Upload Note</Button>}
      />

      {myNotes.length === 0 ? (
        <Card><EmptyState title="No notes shared yet" subtitle="Upload your first set of notes for students to access." /></Card>
      ) : (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Upload Notes" subtitle="Shared instantly with everyone enrolled in the class.">
        <form onSubmit={submit} className="space-y-5">
          ...
          <div>
            <Label>Title</Label>
            <BoxInput required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Week 6 — Query Optimisation" />
          </div>
          <div>
            <Label>File</Label>
            <input
